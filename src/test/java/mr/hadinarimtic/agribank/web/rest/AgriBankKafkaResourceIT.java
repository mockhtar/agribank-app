...
import mr.hadinarimtic.agribank.config.KafkaProperties;
...
class AgriBankKafkaResourceIT {
    ...
    void setup() {
        ...
        AgriBankKafkaResource kafkaResource = new AgriBankKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/agri-bank-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/agri-bank-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
